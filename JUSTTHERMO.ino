...
long lastDownCheck;
long lastModePress;
long lastEntry; //holds the last time thev  RTC was checked
...
void update_Current_Temp() //Function to read temp from onewire thermometer and display it on LCD in the proper location
{
  ...
  {
    heatOn=1;
    digitalWrite(46, LOW);   // turn the LED on (HIGH is the voltage level)
...
void do_Reads(){
  ...
  */
  ...
  minutePower = minutePower + secondPower;
  lastEntry = millis();
}
